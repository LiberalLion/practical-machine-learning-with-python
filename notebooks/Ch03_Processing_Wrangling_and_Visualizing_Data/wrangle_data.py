...
def _random_date(start,date_count):
    ...
    current = start
    while date_count > 0:
        yield current + datetime.timedelta(days=randrange(42))
        date_count-=1
...
def generate_sample_data(row_count=100):
    ...
    data_dict = {
        ...
        'User ID': np.random.permutation(
            np.random.randint(0, row_count, size=int(row_count / 10))
            + user_id_sentinel
        ).tolist()
        * 10,
        'Product ID': np.random.permutation(
            np.random.randint(0, row_count, size=int(row_count / 10))
            + product_id_sentinel
        ).tolist()
        * 10,
        'Quantity Purchased': np.random.permutation(
            np.random.randint(1, 42, size=row_count)
        ),
        'Price': np.round(
            np.abs(np.random.randn(row_count) + 1) * price_sentinel, decimals=2
        ),
        'User Type': np.random.permutation(
            [chr(random.randrange(97, 97 + 3 + 1)) for _ in range(row_count)]
        ),
    }

    # introduce missing values
    for _ in range(int(np.sqrt(row_count))):
...
def describe_dataframe(df=pd.DataFrame()):
    ...
    print(
        ...
        pd.isnull(df).any(1).nonzero()[0].tolist()[:5],
    )
    print("\n")
